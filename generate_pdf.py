...
def build_content():
    ...
    toc_items = [
        ...
        ('  3.6', 'Visualisasi', '9'),
        ('4.', 'DEPLOYMENT & ANTARMUKA APLIKASI', '10'),
        ('  4.1', 'Teknologi Deployment', '10'),
        ('  4.2', 'Arsitektur Aplikasi', '10'),
        ('  4.3', 'Link Akses Aplikasi', '10'),
        ('  4.4', 'Antarmuka Aplikasi', '11'),
        ('5.', 'KESIMPULAN DAN REKOMENDASI', '14'),
        ('  5.1', 'Kesimpulan', '14'),
        ('  5.2', 'Rekomendasi', '14'),
        ('  5.3', 'Keterbatasan', '15'),
        ('6.', 'REFERENSI', '15'),
    ...
    # ══════════════════════════════════════════════════════════
    story.append(section_header('4', 'DEPLOYMENT & ANTARMUKA APLIKASI'))
    story.append(sp(0.3))

    story.append(h2('4.1  Teknologi Deployment'))
    story.append(body(
        'Aplikasi <b>DiabetesSense</b> dibangun menggunakan teknologi open-source dan '
        'di-deploy ke platform cloud sehingga dapat diakses dari mana saja tanpa instalasi.'
    ))
    tech_rows = [
        ('Web Framework',  'Streamlit — membangun antarmuka web interaktif berbasis Python'),
        ('Hosting',        'Streamlit Community Cloud — auto-deploy langsung dari GitHub'),
    ...
    story.append(info_table(tech_rows))
    story.append(caption('Tabel 4.1 — Tech Stack Deployment Aplikasi DiabetesSense'))
    story.append(sp(0.4))

    story.append(h2('4.2  Arsitektur Aplikasi'))
    story.append(body(
        'Aplikasi DiabetesSense terdiri dari <b>6 halaman</b> yang dapat diakses '
        'melalui sidebar navigasi di sisi kiri layar.'
    ))
    arch_rows = [
    ...
    story.append(info_table(arch_rows))
    story.append(caption('Tabel 4.2 — Arsitektur Halaman Aplikasi DiabetesSense'))
    story.append(sp(0.4))

    story.append(h2('4.3  Link Akses Aplikasi'))
    link_rows = [
    ...
    story.append(caption('Tabel 4.3 — Link Akses Aplikasi'))
    story.append(sp(0.5))

    story.append(h2('4.4  Antarmuka Aplikasi'))
    story.append(body(
        'Berikut tampilan antarmuka dari seluruh halaman utama aplikasi DiabetesSense '
        'yang telah berhasil di-deploy dan dapat diakses secara online.'
    ))
    story.append(sp(0.3))

    SS_DIR = BASE_DIR / 'reports' / 'figures' / 'screenshots'
    screenshots = [
        ('01_beranda.png',
         'Gambar 4.1 — Halaman Beranda: Tim pengembang, statistik dataset, dan latar belakang proyek',
         'Halaman Beranda menampilkan identitas tim pengembang (Fahmi & Nathaniela), '
         'statistik ringkas dataset (768 sampel, 8 fitur, 268 kasus DM), serta '
         'latar belakang dan alur pipeline Machine Learning.'),
        ('02_eda.png',
         'Gambar 4.2 — EDA Dashboard: Ringkasan dataset dan preview data interaktif',
         'EDA Dashboard menyediakan 5 tab eksplorasi: Overview (ringkasan & preview tabel), '
         'Distribusi & Univariat, Korelasi & Multivariat, Kualitas Data, dan 5 Key Insights.'),
        ('03_prediksi.png',
         'Gambar 4.3 — Prediksi Diabetes: Form input dan hasil prediksi real-time',
         'Halaman prediksi menyediakan 8 slider input. Hasil menampilkan label '
         '(TIDAK TERDETEKSI / TERDETEKSI DIABETES), probabilitas, tingkat risiko, '
         'dan referensi nilai normal medis.'),
        ('04_evaluasi.png',
         'Gambar 4.4 — Evaluasi Model: Performa Gradient Boosting dan tabel perbandingan 3 model',
         'Menampilkan kartu metrik model terbaik (Accuracy 89,66%, F1 0,8537, AUC-ROC 0,9582), '
         'tabel perbandingan train/val/test, confusion matrix, dan ROC curves.'),
        ('05_interpretasi.png',
         'Gambar 4.5 — Interpretasi & Bisnis: Feature Importance Gradient Boosting',
         'Analisis feature importance menunjukkan Insulin (0,6879) dan Glucose (0,1082) '
         'sebagai fitur terpenting, disertai justifikasi model dan rekomendasi strategis.'),
    ]
    for fname, cap_text, desc_text in screenshots:
        img_path = SS_DIR / fname
        img_obj = try_image(img_path, width=AVAIL_W)
        if img_obj:
            story.append(h3(cap_text.split(' — ')[1].split(':')[0]))
            story.append(body(desc_text))
            story.append(img_obj)
            story.append(caption(cap_text))
            story.append(sp(0.4))
            story.append(PageBreak())

    # ══════════════════════════════════════════════════════════
    # BAB 5 — KESIMPULAN  (dulunya BAB 4)
    # ══════════════════════════════════════════════════════════
    story.append(section_header('5', 'KESIMPULAN DAN REKOMENDASI'))
    story.append(sp(0.3))

    story.append(h2('5.1  Kesimpulan'))
    concl = [
    ...
    story.append(h2('5.2  Rekomendasi'))
    ...
    story.append(h2('5.3  Keterbatasan'))
    lim = [
    ...
    # ══════════════════════════════════════════════════════════
    # BAB 6 — REFERENSI  (dulunya BAB 5)
    # ══════════════════════════════════════════════════════════
    story.append(section_header('6', 'REFERENSI'))
    story.append(sp(0.3))
...
if __name__ == '__main__':
    ...
    print("[OK] PDF berhasil dibuat!")
    print(f"     Lokasi : {OUTPUT_PDF}")
    print(f"     Ukuran : {size_kb:.1f} KB")
    print("     Berisi : Cover + ToC + 6 Bab + Bab 4 (5 Screenshot Aplikasi)")
